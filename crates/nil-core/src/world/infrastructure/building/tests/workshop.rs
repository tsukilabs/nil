...
use crate::military::unit::r#impl::ram::Ram;
use crate::military::unit::{UnitId, WorkshopUnitId};
...
#[test]
fn workshop_recruit_order_deducts_player_resources() -> Result<()> {
  ...
  world.add_workshop_recruit_order(&req(coord))?;

  let remaining_resources = world.ruler(&ruler)?.resources();
  let required_resources = Ram::CHUNK.resources();
...
#[test]
fn cancel_workshop_recruit_order_refunds_player_resources() -> Result<()> {
  let mut world = make_world()?;
  let player = PlayerId::from("Player A");
  spawn_player(&mut world, player.as_str())?;
  ...
  world.add_workshop_recruit_order(&req(coord))?;
  world.cancel_workshop_recruit_order(coord, order_id(&world, coord)?)?;
...
#[test]
fn workshop_recruit_order_deducts_bot_resources() -> Result<()> {
  let mut world = make_world()?;
...
#[test]
fn cancel_workshop_recruit_order_refunds_bot_resources() -> Result<()> {
  let mut world = make_world()?;
  let bot = get_first_bot(&world);
...
#[test]
fn workshop_recruit_order_deducts_precursor_resources() -> Result<()> {
  let mut world = make_world()?;
...
#[test]
fn cancel_workshop_recruit_order_refunds_precursor_resources() -> Result<()> {
  let mut world = make_world()?;
  let precursor = get_first_precursor(&world);
...
fn req(coord: Coord) -> WorkshopRecruitOrderRequest {
  WorkshopRecruitOrderRequest {
    coord,
    unit: WorkshopUnitId::Ram,
    chunks: NonZeroU32::MIN,
  }
}

fn order_id(world: &World, coord: Coord) -> Result<WorkshopRecruitOrderId> {
  let queue = world
    .city(coord)?
    .infrastructure()
    .workshop()
  ...
  queue
    .iter()
    .find(|order| order.squad().id() == UnitId::Ram)
    .unwrap()
    .id()
    .pipe(Ok)
}
