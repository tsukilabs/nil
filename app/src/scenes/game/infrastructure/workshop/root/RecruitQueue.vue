...
import type { Squad, WorkshopRecruitOrderId } from "@tsukilabs/nil-bindings";
import type { WorkshopImpl } from "@/core/model/infrastructure/building/workshop/workshop";

const props = defineProps<{
  workshop: WorkshopImpl;
  loading: boolean;
  onCancel: (id: WorkshopRecruitOrderId) => MaybePromise<void>;
}>();

const { t } = useI18n();

const tableClass = computed(() => {
  return props.workshop.recruitQueue.size === 0 ? "hidden" : null;
});
...
<template>
  <Table :class="cn(tableClass, 'xl:table xl:w-2/5 xl:max-w-[500px] xl:min-w-[250px]')">
    <TableBody>
      <template v-for="order of workshop.recruitQueue" :key="order.id">
